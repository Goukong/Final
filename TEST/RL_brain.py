...
class QLearningTable:
	def __init__(self,POP_SIZE,learning_rate=0.01,reward_decay=0.9,e_greedy=0.8):
		#initailize 
	...
	def choose_best(self):
		...
		state = current_choice.sort_values(axis=0,ascending=False)
		test = np.array(state[0:5].index)
		#pool = (current_choice == np.max(current_choice))
		#action = np.random.choice(current_choice[pool].index,10)
		return test
